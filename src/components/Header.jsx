...
function Header() {
  useEffect(() => {
    ...
    tl.to('.header-text', { opacity: 1, y: -20, x: 0, duration: 1, color: 'black' })
      ...
      .to('.header-text2', { opacity: 1, y: -20, x: 0, duration: 1, color: 'black' })
      .to('.header-text2', { opacity: 0 })
      .to('.image-rating', { display: 'block', ease: 'power1.out', scale: 1, duration: 2 })
      .to('.image-rating', { scale: 0.8, duration: 1 })
      .to('.app-icon', { delay: 1.5, animation: 'none', scale: 3.0, display: 'block', overflow: 'hidden' })
      .to('.app-icon', { delay: 1.5, animation: 'none', scale: 3.0, display: 'block', top: 0, right: 0, left: '120px', onComplete: () => {
          gsap.to('.bg-header', { display: 'block', scale: 1, duration: 1 });
      }})
      .to('.image-rating', { display: 'none' })
      .to('.app-icon', { scale: 0.1, display: 'block', position: 'absolute', overflow: 'hidden', top: '-150px', left: '270px', y: 0 })
      .to('.bg-header', { display: 'block' });
  }, []);
